...
describe('canonical REST routes', () => {
  ...
  it('uses PATCH for partial updates and keeps PUT routes deprecated', () => {
    const routePairs = [
      [practice, 'updatePracticeRoute', 'updatePracticeLegacyRoute', '/{practice_id}'],
      [practiceDetails, 'updatePracticeDetailsRoute', 'updatePracticeDetailsLegacyRoute', '/{practice_id}/details'],
      [
        memberProfiles,
        'updateMemberProfileRoute',
        'updateMemberProfileLegacyRoute',
        '/{practice_id}/members/{user_id}/profile',
      ],
      [preferences, 'updateCategoryPreferencesRoute', 'updateCategoryPreferencesLegacyRoute', '/{category}'],
      [clientIntakes, 'updatePracticeClientIntakeRoute', 'updatePracticeClientIntakeLegacyRoute', '/{uuid}'],
      [matterCore, 'updateMatterRoute', 'updateMatterLegacyRoute', '/{practice_id}/{matter_id}'],
      [matterNotes, 'updateMatterNoteRoute', 'updateMatterNoteLegacyRoute', '/{matter_id}/notes/{note_id}'],
      [timeEntries, 'updateTimeEntryRoute', 'updateTimeEntryLegacyRoute', '/{matter_id}/time-entries/{entry_id}'],
      [expenses, 'updateExpenseRoute', 'updateExpenseLegacyRoute', '/{matter_id}/expenses/{expense_id}'],
      [milestones, 'updateMilestoneRoute', 'updateMilestoneLegacyRoute', '/{matter_id}/milestones/{milestone_id}'],
      [tasks, 'updateMatterTaskRoute', 'updateMatterTaskLegacyRoute', '/{matter_id}/tasks/{task_id}'],
    ];

    for (const [source, canonicalName, legacyName, path] of routePairs) {
      expectRoute(source, canonicalName, 'patch', path);
      expectRoute(source, legacyName, 'put', path, true);
    }
  });
});
