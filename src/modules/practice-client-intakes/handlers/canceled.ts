...
const logger = getLogger(['practice-client-intakes', 'handlers', 'canceled']);
...
export const handlePracticeClientIntakeCanceled = async ({
  ...
  try {
    ...
    logger.info('Practice client intake payment canceled', {
      intakeId: practiceClientIntake.id,
      stripePaymentIntentId: paymentIntent.id,
      eventId,
    });
  } catch (error) {
    logger.error('Failed to handle practice client intake canceled', {
