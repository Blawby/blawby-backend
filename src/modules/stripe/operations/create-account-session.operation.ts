...
const logger = getLogger(['stripe', 'create-account-session-operation']);
...
/**
 * Create a Stripe Account Session for the given organization and requested components (R7).
 * Owns its own tenant check (KTD19) before ever reading the connected account, so a facade
 * caller whose `LegalOperationContext` organization does not match the requested organization is
 * rejected before any database or Stripe I/O — including the "missing or cross-tenant connected
 * account" rejection that must happen before Stripe is ever called.
 */
const createAccountSession = async (
  params: { organizationId: string; components: AllowedComponent[] },
  ctx: LegalOperationContext
): Promise<AccountSessionResponse> => {
  const { organizationId, components } = params;
...
export { createAccountSession, type AccountSessionResponse };
