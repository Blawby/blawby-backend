...
version: 1.0.0
description: |
  Explore a codebase organically to surface architectural friction, find shallow modules,
  and propose deep-module refactors as GitHub issue RFCs. Uses parallel sub-agents to
  design multiple radically different interfaces for the chosen candidate, then recommends
  the strongest approach. Use when user wants to improve architecture, find refactoring
  opportunities, consolidate tightly-coupled modules, or make a codebase more testable
  or AI-navigable.
allowed-tools:
  - run_shell_command
---
...
**If `gh` CLI is available and authenticated** (check with `gh auth status`):
...
Output the complete issue body directly to chat (see the template in [REFERENCE.md](REFERENCE.md)) so the user can copy it.
