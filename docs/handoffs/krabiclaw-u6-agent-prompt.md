...
- Keep each Legal Operation in `src/modules/practice-client-intakes/operations/`, with one domain use case per operation. Operations accept `LegalOperationContext`, perform their own tenant assertion, and must not accept Better Auth sessions, CASL abilities, Hono contexts, D1 clients, or external IDs. This excludes external *identity/auth* IDs specifically; a required provider identifier that is itself the operation's input (e.g. the Stripe Checkout Session ID `getPostPayStatus` resolves by) is not an external ID in this sense.
